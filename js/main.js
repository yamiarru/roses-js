...
function guardarDatos() {
  ...
  // Cierra el modal con ID 'modal-welcome'
  cerrarModal();
...
function cerrarModal() {
  // Asegurarse de que el modal se haya inicializado correctamente
  const modalElement = document.getElementById('modal-welcome');
  const modal = new bootstrap.Modal(modalElement); // Inicializa el modal
...
function generarHtmlProducto(producto) {
  var html = `
    <div class="col-sm col-md-6 col-xl-3 bot1" style="margin-bottom: 20px; display: flex; justify-content: center; align-items: center;">
      <img src="${producto.imagen}" alt="${producto.nombre}" style="width: 100%; border-radius: 8px; margin-bottom: 10px;">
      <div class="description" style="display: flex; justify-content: space-between; align-items: center; width: 100%; max-width: 300px; text-align: center;">
        <div style="display: flex; flex-direction: column; text-align: left;">
          <span>${producto.nombre}</span>
          <span>$${producto.precio}</span>
        </div>
        <button onclick="seleccionarProducto(${producto.id})">Agregar artículo</button>
      </div>
    </div>`;
  $("#combos").append(html);
}
